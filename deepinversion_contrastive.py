...
class DeepInversionClass(object):
    ...
    def get_images(self, net_student=None, targets=None):
        net_teacher2 = self.net_teacher2
        if 'loss_r_feature_layers2' not in self.__dict__:
            self.loss_r_feature_layers2 = []
            for module in net_teacher2.modules():
                if isinstance(module, nn.BatchNorm2d):
                    # import ipdb; ipdb.set_trace()
                    self.loss_r_feature_layers2.append(DeepInversionFeatureHook(module))        
            # import ipdb; ipdb.set_trace()
        ...
        for lr_it, lower_res in enumerate([2, 1]):
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                net_teacher2.zero_grad()
                inputs_jit_for_forward = inputs_jit.clone()
                inputs_jit_for_forward2 = inputs_jit.clone()

                #===============================================================
                outputs_contrastive = net_teacher(inputs_jit_for_forward)
                outputs2_contrastive = net_teacher2(inputs_jit_for_forward2)
                # if (iteration_loc == iterations_per_layer - 1) and lr_it == 1:
                #     from prediction_ranking import get_prediction_ranks,get_classname
                #     import ipdb; ipdb.set_trace()
                outputs_contrastive = self.network_output_function(outputs_contrastive)
                outputs2_contrastive = self.network_output_function(outputs2_contrastive)

                # R_cross classification loss
                # print('check if targets are just indicators')
                # import ipdb; ipdb.set_trace()
                loss = criterion(outputs_contrastive, targets)
                loss2 = criterion(outputs2_contrastive, targets)
                #===============================================================
                if 'keep only misaligned':
                    if 'simonyan saliency':
                        grad_for_saliency = torch.autograd.grad(loss, inputs_jit_for_forward, retain_graph=True,create_graph=True)[0]
                        grad_for_saliency2 = torch.autograd.grad(loss2, inputs_jit_for_forward2, retain_graph=True,create_graph=True)[0]
                    ...
                    def only_keep_misaligned(g):
                        ...
                        if (g- (grad_for_saliency)).abs().max() > 1e-4:
                            import ipdb; ipdb.set_trace()
                        # import ipdb; ipdb.set_trace()
                        ...
                        # return torch.sign(g) * saliency 
                        pass        
                    inputs_jit_for_forward.register_hook(only_keep_misaligned)
                    inputs_jit_for_forward2.register_hook(lambda g:torch.zeros_like(g))                
                #===============================================================
                outputs = net_teacher(inputs_jit)
                outputs2 = net_teacher2(inputs_jit)                
                outputs = self.network_output_function(outputs)
                outputs2 = self.network_output_function(outputs2)
                # # R_cross classification loss
                # # print('check if targets are just indicators')
                # # import ipdb; ipdb.set_trace()
                ...
                loss_r_feature2 = sum([mod.r_feature * rescale[idx] for (idx, mod) in enumerate(self.loss_r_feature_layers2)])
                ...
                loss_aux2 =  self.bn_reg_scale * loss_r_feature2

                if self.adi_scale!=0.0:
                    loss_aux += self.adi_scale * loss_verifier_cig

                loss = self.main_loss_multiplier * (loss - 0.01*loss2) + loss_aux + 0*loss_aux2
