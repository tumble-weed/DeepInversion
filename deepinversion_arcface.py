    ...
    def get_images(self, net_student=None, targets=None):
        ...
        for lr_it, lower_res in enumerate([2, 1]):
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                # top_n_coordinates, concat_out, raw_logits, concat_logits, part_logits, top_n_index, top_n_prob = net_teacher(inputs_jit)
                import ipdb; ipdb.set_trace()
                """
                def load_image(img_path):
                    ...
                    return image                
                """
                print('check if the input to the network is correctly normalized etc? ')
                from arcface_pytorch.test import get_featurs, get_feature_dict,cosin_metric
                features,cnt = get_featurs(net_teacher,inputs_jit,batch_size=self.bs)
                print(colorful.red("check what features looks like"))
                print(colorful.red("i will need some reference features. download lfw and choose some identities and pick some images from its training data"))
                import ipdb; ipdb.set_trace()
