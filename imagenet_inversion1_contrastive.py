...
def validate_one(input, target, model,modelname='verifier'):
    ...
    print(f"{modelname} accuracy: ", prec1.item())
...
def run(args):
    ...
    from deepinversion_contrastive import DeepInversionClass
    ...
    net_teacher2 = torchvision.models.resnet18(pretrained=True).to(device)
    net_teacher2.eval()
    # check accuracy of verifier
    if args.verifier:
        def hook_for_display(x,y):
            validate_one(x, y, net_verifier,modelname="verifier")
            validate_one(x, y, net,modelname="teacher")
            validate_one(x, y, net_teacher2,modelname="teacher2")
    else:
    ...
    DeepInversionEngine.net_teacher2 =net_teacher2
    net_student=None
    if args.adi_scale != 0:
