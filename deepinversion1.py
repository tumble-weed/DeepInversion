...
class DeepInversionClass(object):
    def __init__(self, bs=84,
        ...
        prefix = path
        self.prefix = prefix
        if os.environ.get('MAHENDRAN',False) == '1':
            self.prefix = self.prefix + '_mahendran'
    ...
    def get_images(self, net_student=None, targets=None):
        ...
        for lr_it, lower_res in enumerate([2, 1]):
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                if False and (iteration_loc == iterations_per_layer - 1) and lr_it == 1:
                ...
                if iteration % save_every==0 and (save_every > 0):
                    os.makedirs(os.path.join(self.prefix,'best_images'),exist_ok=True)
                    if local_rank==0:
                        if False:
                            import pickle
